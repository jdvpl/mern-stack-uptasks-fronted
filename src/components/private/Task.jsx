...
const Task = ({task}) => {
  ...
  const {name,finished,description,priority,dateDelivery,_id,finishedBy} = task;
  ...
  return (
    <div className={`${!finished?'bg-red-50':'bg-green-50'} border-b p-5 flex justify-between items-center  `} key={_id}>
      <div className="flex flex-col items-start">
        ...
        {finished && <p className="text-xs bg-green-600 uppercase p-1 rounded-lg text-white font-bold">Finished by: {finishedBy.name}</p>}
      </div>
